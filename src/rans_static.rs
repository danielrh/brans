...
fn safe_rans_uncompress_O1h(in_0: &[c_uchar],
    ...
    let pptr = cp;
    let mut ptr_offset = 0u32;
    u8Rans64DecInit(&mut R0,
                  pptr, &mut ptr_offset);
    
    u8Rans64DecInit(&mut R1,
                  pptr, &mut ptr_offset);
    u8Rans64DecInit(&mut R2,
                  pptr, &mut ptr_offset);
    u8Rans64DecInit(&mut R3,
                  pptr, &mut ptr_offset);
    ////eprintln!("{:x}\n{:x}\n{:x}\n{:x}", R[0], R[1], R[2], R[3]);
    let mut isz4: c_int = out_sz >> 2i32;
    let mut l0: c_int = 0i32;
    ...
    for (index, out0) in out_buf.iter_mut().enumerate() {
        ...
        *out0 = c;
        ...
        R0 =
            (start_freq.freq as
             c_ulong).wrapping_mul(R0 >> 12);
        R0 =
            (R0 as
             c_ulong).wrapping_add(m.wrapping_sub(start_freq.start) as u64);
        let lt_1_sl_31 = (R0 & 0xffff_ffff_8000_0000);
        
        if lt_1_sl_31 == 0 {
            u8Rans64DecForceRenorm(&mut R0,
                              pptr, &mut ptr_offset);
        }
        let tmp = R0;
        R0 = R1;
